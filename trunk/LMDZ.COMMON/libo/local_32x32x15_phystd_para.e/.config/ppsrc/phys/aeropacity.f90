      Subroutine aeropacity(ngrid,nlayer,nq,pplay,pplev,pq, &
       ...
       use callkeys_mod, only: aerofixco2,aerofixh2o,aerofixash,kastprof, &
		...
		pres_bottom_strato,pres_top_strato,obs_tau_col_strato,  &
                tau_nh3_cloud, pres_nh3_cloud,rho_volc
...
!$OMP THREADPRIVATE(i_co2ice,i_h2oice,i_volc)
	...
	if (iaero_ash.ne.0) then
          print*,'iaero_ash= ',iaero_ash 
        endif
...
!             Vertical scaling function
         ...
         DO l=1,nlayer-1
            DO ig=1,ngrid
! Here we are getting a certain opacity at particular wavelength. 
! Everywhere on the planet is specified by random tau value. 
! Tracer not linked to aerosols. See Kerber et al. 2015
! Comment out if you want aerosol tied to tracer. I (Saira) made
! the variable h2so4tau to replace the default value of 1.
!               aerosol(ig,l,iaer) = max(1E-20, &
!                          h2so4tau &
!                       *  pplev(ig,1) / pplev(ig,1) &
!                       *  aerosol(ig,l,iaer) &
...
! Below the aerosol is linked to the tracer "pq(ig,1,i_h2so4)"
...
!       2. Opacity calculation
            if (noaero) then ! aerosol set to zero
             aerosol(1:ngrid,1:nlayer,iaer)=0.0
            elseif (aerofixash.or.(i_volc.eq.0)) then !  Ash cloud prescribed
               aerosol(1:ngrid,1:nlayer,iaer)=1.e-9
               !aerosol(1:ngrid,12,iaer)=4.0 ! single cloud layer option
            else
               DO ig=1, ngrid
...
!                     aerosol(ig,l,iaer) = min(aerosol(ig,l,iaer),L_TAUMAX) !largest optical depth


                  ENDDO
               ENDDO
            end if ! if fixed or varying
      end if ! if ash aerosols   
