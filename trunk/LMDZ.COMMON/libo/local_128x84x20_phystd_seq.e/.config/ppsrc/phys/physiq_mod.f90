...
! TENDENCIES due to various processes :
      ...
      real icesurf(ngrid,nq)		  ! accumulated surface ice
...
!            call volcano(nq,ngrid,nlayer,pplev,pu,pv,pt,zzlev,zdqvolc,ivolc)
            call volcano(ngrid,nlayer,pplev,pu,pv,pt,zzlev,zdqvolc,nq,massarea,cell_area)
!           do l = 1,nlayer
...
!               dqsurf(ig,iq)=dqsurf(ig,iq)+zdqschim(ig,iq)
  ...
  ! -----------------------------
	   ...
	   icesurf(1:ngrid,1:nq) = qsurf(1:ngrid,1:nq) + abs(ptimestep*dqsurf(1:ngrid,1:nq))
...
!                          'kg m^-2',2,qsurf(1,iq) )                          
	    ...
	    if(hydrology)then
	      call writediagfi(ngrid,"icesurf","accumulated surface ice","kg/m2",2,icesurf)
	    endif
