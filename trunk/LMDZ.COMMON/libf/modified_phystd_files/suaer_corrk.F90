...
! added by LK
 ...
 if (iaer.eq.iaero_ash) then
         print*, 'naerkind= ash', iaer

!     visible
         file_id(iaer,1) = 'optprop_ash_vis_n50.dat'
         !lamrefir(iaer)=  doesn't exist?
         lamrefvis(iaer)=0.67e-6   ! no idea, must find
!     infrared
         file_id(iaer,2) = 'optprop_ash_ir_n50.dat'
         !lamrefir(iaer)=12.1E-6   ! 825cm-1 TES/MGS
         lamrefir(iaer)=9.3E-6 ! no idea, must find
! added by Saira Hamid
       endif
